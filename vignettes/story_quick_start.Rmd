...
We note that the targeted `Events` are approximately what would be proposed with the @Schoenfeld formula:
...
gs_prob <- function(theta, upper = gs_b, lower = gs_b, upar, lpar, info, r = 18){
   ...
   for(k in seq_along(info)){
     if(k == 1){
        upperProb[1] <- if(Zupper[1] < Inf) {pnorm( sqrt(info[1]) * (theta[1] - Zupper[1] / sqrt(info0[1])))}else{0}
        lowerProb[1] <- if(Zlower[1] > -Inf){pnorm(-sqrt(info[1]) * (theta[1] - Zlower[1] / sqrt(info0[1])))}else{0}
