...
vignette: >
  ...
  %\VignetteIndexEntry{Usage of AHR}
---
...
`gs_b()` can be used to derive spending boundary in group sequential design. It is usually used in the `upper = ...` and `lower = ...` arguments in 
+ `gs_power_npe()`
+ `gs_design_npe()`
+ `gs_power_ahr()`
+ `gs_design_ahr()`
