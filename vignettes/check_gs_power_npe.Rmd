...
If one inputs `info` in `lpar`

```{r}
gs_power_npe(theta = 0, 
             info = 50,
             upper = gs_b, 
             upar = list(par = qnorm(0.025)),
             lower = gs_spending_bound, 
             lpar = list(par = list(sf = gsDesign::sfLDOF, param = NULL, total_spend = 0.1), 
                         info = 40)) %>% gt()
```
