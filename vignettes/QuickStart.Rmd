...
subtitle: "Using average hazard ratio"
...
vignette: |
  %\VignetteIndexEntry{Quick Start for NPH Sample Size and Power (Copy)}
...
design1s <- gs_design_ahr(
  ...
  # specify spending function and total Type I error
...
```{r}
summary(design1s) %>% as_gt(title="1-sided group sequential bound using AHR method",
                            subtitle="Lan-DeMets spending to approximate O'Brien-Fleming bound") 
```


Notes to Yujie:
1) How should we make it clear that info_scale= 2 is the default?
2) Have we documented info_scale in a vignette?
3) Note that total Type I error above is 0.0252 rather than the specified 0.025. I suspect this is an issue with gs_spending_bound(). See following gsDesign example.
...
ggplot(
  data = design2ss$analysis %>% 
    left_join(design2ss$bounds, by="Analysis"), 
