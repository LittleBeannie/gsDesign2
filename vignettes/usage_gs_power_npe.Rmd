...
vignette: >
  ...
  %\VignetteIndexEntry{Usage of gs_power_npe}
---
...
gs_power_npe(
  ...
  lower = gs_b,
  lpar = list(par = c(qnorm(.1), -Inf, -Inf))) %>% gt()
```
...
gs_power_npe(
  ...
  lpar = list(par = -(x %>% filter(Bound == "Upper"))$Z)) %>% gt()
```
